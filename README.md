...
The 11.7-foot average shot distance is the detail that doesn't get enough attention. That's deep mid-range, not a rim-runner's number and not a perimeter specialist's. It's a player who's spent two decades operating in the contested middle of the court and been effective there.

---

## Dead-ball vs 3PT era

The league's transformation didn't happen in a single offseason, but 2014-15, when Curry's Warriors won the title, is the cleanest dividing line.

![Shot Distance Evolution](analysis/charts/shot_distance_evolution.png)
...
![3-Point Rate Comparison](analysis/charts/three_point_rate.png)
...
![FG% by Tenure](analysis/charts/fg_by_tenure.png)
...
By his final Cavs I season, he was averaging 12.8 feet per shot, longer than his current average, which means more pull-up jumpers and fewer plays finishing at the rim. The efficiency was solid for the era but not yet elite.
...
Home/away split: 55.3% at home, 53.4% away. The gap is small, which means he wasn't getting carried by friendly rims.
...
His clutch FG% during this tenure, 50.8%, was the best of any chapter.
...
![Clutch Comparison](analysis/charts/clutch_comparison.png)
...
Two things worth keeping in mind when looking at that gap. First, he takes significantly more threes in clutch situations, 31.5% vs 22.9% in regular play. Shifting to a lower-percentage shot pulls FG% down arithmetically even if the shot selection is defensible. The points-per-shot gap is smaller than the FG% gap: 1.044 clutch vs 1.103 regular.

Second, on final possessions, last 24 seconds and actual game-winners, he shot 38.6% on 497 attempts. That's a real sample size, and 38.6% on contested shots with the defense built specifically to stop you is not a collapse. Most NBA stars shoot worse in the same situation.
...
![Top 10 Opponents](analysis/charts/top_opponents.png)
*Figure 5: Most-faced opponents by career shot attempts. Color indicates FG%, green is efficient, red is tougher.*

LeBron's most-faced opponents are almost all Eastern Conference teams from his Cleveland and Miami years. Chicago (1,385 shots, 67 games), Indiana (1,316), Atlanta (1,281), and Boston (1,257) sit at the top. Multiple playoff series against those teams account for the volume.
...
The Clippers (47.4% on 998 shots) are his worst large-sample matchup. The Detroit number (47.6% on 1,250 shots) goes back to the Pistons' physical playoff defense from 2005-2008, it clearly worked. Houston's scheme under multiple coaches has consistently created problems.

Home/away: LeBron shoots about 1.5-2 points better at home across opponents. Typical for NBA stars.

---

## Shot Selection Evolution

![Shot Distribution by Era](analysis/charts/shot_distribution.png)
*Figure 6: Shot zone distribution by era. Long mid-range attempts were nearly cut in half.*

### The Court Map

![Shot Chart](analysis/charts/shot_chart_heatmap.png)
...
**Long Mid-range** (17-23 ft): Dropped from 23.7% to 11.5%. Those pull-up 20-footers were largely gone from his chart by 2016. The Cavs coaching staff and his own film work aligned on shot quality.

**Three-pointers** (24+ ft): Up from 17.0% to 26.4%.

---

## Career Trajectory

![Career FG% Trend](analysis/charts/career_fg_trend.png)
...
The Lakers years are consistent. The 2023-24 season (54.0%) was his best as a Laker from an efficiency standpoint, and at 39 years old, that's not a maintenance story.
...
![Best Seasons](analysis/charts/best_seasons.png)
...
By the time he got to Miami, his shot volume dropped and his efficiency went up sharply. That's what happens when you play alongside other threats who force defenses to respect multiple players.
...
The more telling number is **2017-18 at #2**. He scored 1,863 field-goal points on 1,580 shots at 54.2%, 257 fewer shots than 2005-06 but nearly the same total output, purely because of better conversion. That's LeBron at 33, in his final Cleveland season, putting up 22.7 points per game from the field while taking fewer shots. Four of the top five are from Cavs I; 2017-18 is the outlier that shows what efficiency can do.

### By Effectiveness: The Miami Peak and the 2023-24 Surprise

The ranking by FG% tells a different story from volume or scoring. The top two seasons are both Miami: **2013-14 (56.7%) and 2012-13 (56.5%)**. These are the seasons where shot selection, system fit, and physical peak all lined up. He was taking shots at the rim and at the short mid-range, avoiding the long two-pointers, and finishing at a rate that almost no high-usage player in NBA history has matched for consecutive seasons.
...
The 2023-24 season deserves attention. At 39 years old, LeBron shot 54.0% and generated 1.197 points per shot, third-highest of his entire career, behind only those two Miami peaks. He did it while taking 28.6% of his shots from three at 41.0% from deep. The approach is different from Miami; the output is nearly identical.
...
- **Staging**: 24 parquet files (one per season) loaded into DuckDB, columns standardized, dates parsed, opponent teams derived from home/visitor abbreviations, seconds remaining calculated
...
- matplotlib 3.10, seaborn 0.13, numpy
...
```
lebron-shot-analytics/
├── data/
│   └── raw/                        # parquet files, one per season (gitignored)
├── lebron_analytics/               # dbt project
│   ├── models/
│   │   ├── sources.yml
│   │   ├── staging/                # stg_lebron_shots
│   │   ├── intermediate/           # era, tenure, clutch, enriched
│   │   └── marts/                  # era, tenure, clutch, opponent, best_seasons
│   ├── seeds/
│   │   └── team_abbreviations.csv
│   └── dbt_project.yml
├── notebooks/
│   ├── lebron_analysis.ipynb       # main analysis
│   ├── lebron_best_season.ipynb    # best seasons breakdown
│   ├── visualizations.ipynb        # generates all charts
│   ├── explore_data.ipynb          # raw data exploration
│   ├── query_results.ipynb         # mart table summaries
│   ├── query_intermediate.ipynb    # intermediate model verification
│   └── check_schemas.ipynb         # database schema inspection
├── scripts/
│   ├── extract.py                  # fetches raw shot data
│   └── load_data.py                # loads parquet files into DuckDB
├── analysis/
...
# 3. Load raw parquet files into DuckDB
...
To regenerate charts, run `notebooks/visualizations.ipynb`. To browse dbt docs:
